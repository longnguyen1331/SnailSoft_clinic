...
var PhieuKeToanStatus = function () {
    let dtTable = null;
    ...
    let validator = FormValidation.formValidation(modal_form[0], {
        fields: {
            kt_modal_edit_form_code: { validators: { notEmpty: { message: "vui lòng nhập mã trạng thái" } } },
            kt_modal_edit_form_name: { validators: { notEmpty: { message: "vui lòng nhập tên trạng thái" } } }
        },
        plugins: {
    ...
    let initialComponents = () => {
        ...
        $('[name="btnCreate"]').click(function (e) {
            ...
            editingDataRow = null;
            modal_header_text.text("Thêm một trạng thái");
                ...
                validator.validate().then(function (e) {
                    "Valid" == e ?
                        (
                            ...
                            App.sendDataToURL("/PhieuKeToanStatus/Save", data, "POST")
                            .then(function (res)
                            {
    ...
    let initialDatatable = function () {
        ...
        datatableOption.ajax.url = "/PhieuKeToanStatus/DataTableGetList";
        ...
        datatableOption.columns = [
            ...
            { "data": "code", "name": "code", "autoWidth": true, "title": "Mã trạng thái" },
            { "data": "name", "name": "name", "autoWidth": true, "title": "Tên trạng thái" },
            {
        ...
        $('#dtTable tbody').on('click', 'a.edit', function (e) {
            ...
            modal_header_text.text("Cập nhật loại trạng thái");
    ...
    function deleteDataRows(dataRows) {
        App.deleteDataConfirm({ ids: dataRows.map((item) => item.id) }, "/PhieuKeToanStatus/DeleteByIds", dtTable, null)
        .then(function () {
            dtTable.draw();
            App.showHideButtonDelete(false);
        });
