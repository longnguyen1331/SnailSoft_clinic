...
var AttributeValue = function () {
    ...
    let modal = $("#kt_modal_edit"),
        ...
        modal_form_submit_text = $("#kt_modal_edit_submit");
    let editingDataRow = null;

    let validator = FormValidation.formValidation(modal_form[0], {
        fields: {
            kt_modal_edit_form_name: { validators: { notEmpty: { message: "vui lòng nhập giá trị thuộc tính" } } },
            kt_form_edit_attribute: { validators: { notEmpty: { message: "vui lòng chọn thuộc tính" } } },
        },
        plugins: {
    ...
    let initialComponents = () => {
        ...
        $('button[name="btnCreate"]').click(function (e) {
            ...
            editingDataRow = null;
            modal_header_text.text("Thêm một giá trị thuộc tính");
        ...
        $('[name="kt_form_edit_attribute"]').on("change", function () {
            validator.revalidateField("kt_form_edit_attribute");
        });
    ...
    let initialDatatable = function () {
        ...
        datatableOption.ajax.url = "/AttributeValue/DataTableGetList";
        ...
        datatableOption.columns = [
            ...
            { "data": "name", "name": "name", "autoWidth": true, "title": "Giá trị" },
            { "data": "attributeName", "name": "attributeName", "autoWidth": true, "title": "Thuộc tính" },
            {
                "data": "description", "name": "description", "autoWidth": true, "title": "Ghi chú", "render": function (data, type, full, meta) {
        ...
        $('#dtTable tbody').on('click', 'a.edit', function (e) {
            ...
            $('input[data-field="Description"]').val(selectedDataRow.description);
            $('input[data-field="Name"]').val(selectedDataRow.name);
            $('[name="kt_form_edit_attribute"]').append('<option selected value="' + selectedDataRow.attributeId + '">' + selectedDataRow.attributeName + '</option>').trigger('change');
            modal_header_text.text("Cập nhật giá trị thuộc tính");
    ...
    function initSelect2() {
        App.initSelect2Base($('[name="kt_form_edit_attribute"]'), '/Attribute/Filter');
    }

    function deleteDataRows(dataRows) {
        App.deleteDataConfirm({ ids: dataRows.map((item) => item.id) }, "/AttributeValue/DeleteByIds", dtTable, null)
            .then(function () {
                dtTable.draw();
                App.showHideButtonDelete(false);
            });
    ...
    let resetForm = () => {
        modal_form[0].reset();
        $('[name="kt_form_edit_attribute"]').val(null).trigger('change.select2');
        setTimeout(function () { $('input[name="kt_modal_edit_form_name"]').focus() }, 500);
    }
    return {
        // public functions
