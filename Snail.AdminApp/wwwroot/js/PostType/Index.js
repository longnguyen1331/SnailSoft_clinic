...
var PostType = function () {
    ...
    let validator = FormValidation.formValidation(modal_form[0], {
        fields: {
            kt_modal_edit_form_name: { validators: { notEmpty: { message: "vui lòng nhập tên chuyên mục" } } }
        },
        plugins: {
    ...
    let initialComponents = () => {
        ...
        $('[name="btnCreate"]').click(function (e) {
            ...
            editingDataRow = null;
            modal_header_text.text("Thêm một chuyên mục");
                ...
                validator.validate().then(function (e) {
                    "Valid" == e ?
                        (
                            ...
                            App.sendDataToURL("/PostType/Save", data, "POST")
                            .then(function (res)
                            {
    ...
    let initialDatatable = function () {
        ...
        datatableOption.ajax.url = "/PostType/DataTableGetList";
        ...
        datatableOption.columns = [
            ...
            { "data": "name", "name": "name", "autoWidth": true, "title": "Tên chuyên mục" },
            {
        ...
        $('#dtTable tbody').on('click', 'a.edit', function (e) {
            ...
            modal_header_text.text("Cập nhật loại chuyên mục");
    ...
    function deleteDataRows(dataRows) {
        App.deleteDataConfirm({ ids: dataRows.map((item) => item.id) }, "/PostType/DeleteByIds", dtTable, null)
        .then(function () {
            dtTable.draw();
            App.showHideButtonDelete(false);
        });
