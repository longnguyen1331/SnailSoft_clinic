...
var DocTemplateType = function () {
    ...
    let validator = FormValidation.formValidation(modal_form[0], {
        fields: {
            kt_modal_edit_form_code: { validators: { notEmpty: { message: "vui lòng nhập mã loại mẫu in" } } },
            kt_modal_edit_form_name: { validators: { notEmpty: { message: "vui lòng nhập tên loại mẫu in" } } }
        },
        plugins: {
    ...
    let initialComponents = () => {
        ...
        $('[name="btnCreate"]').click(function (e) {
            ...
            editingDataRow = null;
            modal_header_text.text("Thêm một loại mẫu in");
                ...
                validator.validate().then(function (e) {
                    "Valid" == e ?
                        (
                            ...
                            App.sendDataToURL("/DocTemplateType/Save", data, "POST")
                            .then(function (res)
                            {
    ...
    let initialDatatable = function () {
        ...
        datatableOption.ajax.url = "/DocTemplateType/DataTableGetList";
        ...
        datatableOption.columns = [
            ...
            { "data": "code", "name": "code", "autoWidth": true, "title": "Mã loại mẫu in" },
            { "data": "name", "name": "name", "autoWidth": true, "title": "Tên loại mẫu in" },
            {
        ...
        $('#dtTable tbody').on('click', 'a.edit', function (e) {
            ...
            modal_header_text.text("Cập nhật loại loại mẫu in");
    ...
    function deleteDataRows(dataRows) {
        App.deleteDataConfirm({ ids: dataRows.map((item) => item.id) }, "/DocTemplateType/DeleteByIds", dtTable, null)
        .then(function () {
            dtTable.draw();
            App.showHideButtonDelete(false);
        });
