...
namespace Snail.AdminApp.Controllers
{
    ...
    {
        private readonly IWebsiteMenuApiClient _websiteMenuApiClient;
        public WebsiteMenuController(IWebsiteMenuApiClient websiteMenuApiClient)
        {
            _websiteMenuApiClient = websiteMenuApiClient;
        }

        public IActionResult Index()
        {
            var model = new WebsiteMenuViewModel();
            ...
            model.Breadcrumbs = new List<string>() { "Quản trị Website", "Menu" };
        ...
        public async Task<IActionResult> DataTableGetList(int? draw, int? start, int? length, string textSearch)
        {
            ...
            var request = new ManageWebsiteMenuPagingRequest()
            {
            ...
            var websiteMenuApiClient = await _websiteMenuApiClient.GetManageListPaging(request);

            return Json(new
            {
                draw = draw,
                recordsFiltered = websiteMenuApiClient.TotalRecords,
                recordsTotal = websiteMenuApiClient.TotalRecords,
                data = websiteMenuApiClient.Items
            });
        }
        ...
        public async Task<IActionResult> DeleteByIds([FromBody] DeleteRequest request)
        {
            ...
            var result = await _websiteMenuApiClient.DeleteByIds(request);
        ...
        [HttpPost]
        public async Task<IActionResult> Save([FromBody] AddEditRequest<WebsiteMenuRequest> rq)
        {
            ...
            if (rq != null)
            {
                ...
                result = await _websiteMenuApiClient.AddOrUpdateAsync(rq.Data);
            }
            else
            {
