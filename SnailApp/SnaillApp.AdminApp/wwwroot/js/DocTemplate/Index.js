...
var DocTemplate = function () {
    ...
    let initialComponents = () => {
        ...
        $("#filterByDocTemplateTypeId").change(function () {
            dtTable.draw();
        });

        App.initSelect2Base($('#filterByDocTemplateTypeId'), '/DocTemplateType/Filter');
    };

    let initialDatatable = function () {
        var datatableOption = initialDatatableOption();
        datatableOption.ajax.url = "/DocTemplate/DataTableGetList";
        datatableOption.ajax.data = {
            textSearch: function () {
                return $('#dtTableSearch').val();
            },
            docTemplateTypeId: function () {
                return $('#filterByDocTemplateTypeId').val();
            }
        };
        ...
        datatableOption.columns = [
            ...
            { "data": "name", "name": "name", "autoWidth": true, "title": "Tên mẫu in" },
            {
                "data": "isDefault", "name": "isDefault", "autoWidth": true, "title": "Mặc định",
                "render": function (data, type, full, meta) {
                    let html = '<div class="form-check form-check-sm form-check-custom form-check-solid">' +
                        '<input class="form-check-input" type="checkbox" disabled value ="" ' + (data == true ? 'checked' : '') + '>' +
                                '</div>';
                    return html;
                }
            },
        ...
        $('#dtTable tbody').on('click', 'a.edit', function (e) {
            ...
            if (selectedDataRow != null) {
                window.location.href = "/DocTemplate/Edit?id=" + selectedDataRow.id;
            }
        });
    ...
    function deleteDataRows(dataRows) {
        App.deleteDataConfirm({ ids: dataRows.map((item) => item.id) }, "/DocTemplate/DeleteByIds", dtTable, null)
        .then(function () {
            dtTable.draw();
            App.showHideButtonDelete(false);
        });
