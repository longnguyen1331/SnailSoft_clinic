...
var Attribute = function () {
    ...
    let validator = FormValidation.formValidation(modal_form[0], {
        fields: {
            kt_modal_edit_form_name: { validators: { notEmpty: { message: "vui lòng nhập tên thuộc tính" } } }
        },
        plugins: {
    ...
    let initialDatatable = function () {
        ...
        datatableOption.ajax.url = "/Attribute/DataTableGetList";
    ...
    function deleteDataRows(dataRows) {
        App.deleteDataConfirm({ ids: dataRows.map((item) => item.id) }, "/Attribute/DeleteByIds", dtTable, null)
            .then(function () {
                dtTable.draw();
                App.showHideButtonDelete(false);
            });
