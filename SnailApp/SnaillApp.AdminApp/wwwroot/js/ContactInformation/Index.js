...
var ContactInformation = function () {
    ...
    let validator = FormValidation.formValidation(modal_form[0], {
        fields: {
            kt_modal_edit_form_code: { validators: { notEmpty: { message: "vui lòng nhập mã " } } },
                ...
                validator.validate().then(function (e) {
                    "Valid" == e ?
                        (
                            ...
                            modal_form.find("select, textarea, input").each((index, el) => {
                                ...
                                if (fieldName) {
                                    ...
                                    switch (fieldName) {
                                        case "IsDefault":
                                            if ($(el).val() == "on") {
                                                result[$(el).data("field")] = true;
                                            }
                                            else {
                                                result[$(el).data("field")] = false;
                                            }
                                            break;
                            ...
                            App.sendDataToURL("/ContactInformation/Save", data, "POST")
    ...
    let initialDatatable = function () {
        ...
        datatableOption.ajax.url = "/ContactInformation/DataTableGetList";
        ...
        datatableOption.columns = [
            ...
            { "data": "facebookLink", "name": "facebookLink", "autoWidth": true, "title": "Facebook" },
            { "data": "instagramLink", "name": "instagramLink", "autoWidth": true, "title": "InstagramLink" },
            { "data": "youtubeLink", "name": "youtubeLink", "autoWidth": true, "title": "YoutubeLink" },
            {
                "data": "isDefault", "name": "isDefault", "autoWidth": true, "title": "Mặc định",
                "render": function (data, type, full, meta) {
                    let html = '<div class="form-check form-check-sm form-check-custom form-check-solid">\
                                    <input class="form-check-input" disabled type ="checkbox" '+ (data == true ? 'checked' : '') + ' value ="" >\
                                </div>';
                    return html;
                }
            },
        ...
        $('#dtTable tbody').on('click', 'a.edit', function (e) {
            ...
            $('input[data-field="FacebookLink"]').val(selectedDataRow.facebookLink);
            $('input[data-field="InstagramLink"]').val(selectedDataRow.instagramLink);
            $('input[data-field="YoutubeLink"]').val(selectedDataRow.youtubeLink);
            $('input[data-field="IsDefault"]').prop('checked', editingDataRow.isDefault);
    ...
    function deleteDataRows(dataRows) {
        App.deleteDataConfirm({ ids: dataRows.map((item) => item.id) }, "/ContactInformation/DeleteByIds", dtTable, null)
        .then(function () {
            dtTable.draw();
            App.showHideButtonDelete(false);
        });
