...
var Post = function () {
    ...
    let initialComponents = () => {
        ...
        $("#filterByPostTypeId").change(function () {
            dtTable.draw();
        });

        App.initSelect2Base($('#filterByPostTypeId'), '/PostType/Filter');
    };

    let initialDatatable = function () {
        var datatableOption = initialDatatableOption();
        datatableOption.ajax.url = "/Post/DataTableGetList";
        datatableOption.ajax.data = {
            textSearch: function () {
                return $('#dtTableSearch').val();
            },
            postTypeId: function () {
                return $('#filterByPostTypeId').val();
            }
        };
        ...
        datatableOption.columns = [
            ...
            { "data": "name", "name": "name", "autoWidth": true, "title": "Tiêu đề" },
            { "data": "viewCount", "name": "viewCount", "autoWidth": true, "title": "Lượt xem" },
            {
                "data": "", "name": "", "autoWidth": true, "title": "Trạng thái", "render": function (data, type, full, meta) {
                    let html = '<span class="badge ' + (full.postStatus.name === "Đang hoạt động" ? "badge-light-success" :"badge-light-danger")+'">' + full.postStatus.name+'</span>';
                    return html;
                }
            },
            {
                "data": "", "name": "", "autoWidth": true, "title": "Chuyên mục", "render": function (data, type, full, meta) {
                    let html = '<span>' + full.postType.name + '</span>';
                    return html;
                }
            },
            {
        ...
        $('#dtTable tbody').on('click', 'a.edit', function (e) {
            ...
            if (selectedDataRow != null) {
                window.location.href = "/Post/Edit?id=" + selectedDataRow.id;
            }
        });
    ...
    function deleteDataRows(dataRows) {
        App.deleteDataConfirm({ ids: dataRows.map((item) => item.id) }, "/Post/DeleteByIds", dtTable, null)
        .then(function () {
            dtTable.draw();
            App.showHideButtonDelete(false);
        });
