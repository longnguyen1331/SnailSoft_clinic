...
var WebsiteMenu = function () {
                ...
                validator.validate().then(function (e) {
                    "Valid" == e ?
                        (
                                    ...
                                    switch (fieldName) {
                                        case "ParentId":
                                            if (!isNaN(parseInt($(el).val()))) {
                                                result[$(el).data("field")] = parseInt($(el).val());
                                            }
                            ...
                            App.sendDataToURL("/WebsiteMenu/Save", data, "POST")
                            .then(function (res)
                            {
    ...
    let initialDatatable = function () {
        ...
        datatableOption.ajax.url = "/WebsiteMenu/DataTableGetList";
        ...
        datatableOption.columns = [
            ...
            { "data": "sortOrder", "name": "sortOrder", "autoWidth": true, "title": "Số thứ tự" },
            { "data": "parentId", "name": "parentId", "autoWidth": true, "title": "Id của Menu cha" },
            { "data": "link", "name": "link", "autoWidth": true, "title": "Link" },
            { "data": "websiteMenuType", "name": "websiteMenuType", "autoWidth": true, "title": "Loại menu" },            
            {
                "data": "icon", "name": "icon", "autoWidth": true, "title": "Icon",
                "render": function (data, type, full, meta) {
                    return '<div><i class="' + data + '"></i></div>';
                }
            },
            {
        ...
        $('#dtTable tbody').on('click', 'a.edit', function (e) {
            ...
            $('input[data-field="ParentId"]').val(selectedDataRow.parentId);
            $('input[data-field="Link"]').val(selectedDataRow.link);
            $('select[data-field="WebsiteMenuType"]').val(selectedDataRow.websiteMenuType);
            $('input[data-field="Icon"]').val(selectedDataRow.icon);
            
            modal_header_text.text("Cập nhật loại ");
    ...
    function deleteDataRows(dataRows) {
        App.deleteDataConfirm({ ids: dataRows.map((item) => item.id) }, "/WebsiteMenu/DeleteByIds", dtTable, null)
        .then(function () {
            dtTable.draw();
            App.showHideButtonDelete(false);
        });
