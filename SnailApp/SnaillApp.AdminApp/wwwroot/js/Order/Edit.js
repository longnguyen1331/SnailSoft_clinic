...
var Order = function () {
    ...
    let onAddProduct = (e) => {
        if ($('#product_' + e.value + '').length <= 0) {
            ...
            let html = '<tr id="product_' + e.value + '">\
                ...
                <td>\
                     <input type="text" class="form-control form-control-solid tien-VND inputPriceProductDetail" data-id="'+ e.value + '" autocomplete="off" name="null-0" data-field="BuyingPrice" value="' + e.buyingPrice + '">\
                </td>\
                <td>\
                    <input type="text" class="form-control form-control-solid tien-VND inputTotalProductDetail" data-id="'+ e.value + '"  disabled name="null-0" data-field="Total" value="' + e.buyingPrice +'">\
                </td>\
    ...
    let refreshTotal = () => {
        let tongTien = 0, tienGiam = 0, 
            khachDua = $('#kt_modal_edit_form_totalPaid').autoNumeric('get'),
            giamGia = $('#kt_modal_edit_form_totalPercent').val();
        ...
        $('#kt_modal_edit_form_total_product').autoNumeric('set', tongTien);
        tienGiam = (tongTien * giamGia / 100).toFixed(2);
        $('#kt_modal_edit_form_totalCommission').autoNumeric('set', tienGiam);
        $('#kt_modal_edit_form_totalMoney').autoNumeric('set', tongTien - tienGiam);
        $('#kt_modal_edit_form_totalDebt').autoNumeric('set', tongTien - tienGiam - khachDua);
    }

    function initSelect2() {

        $('[name="kt_form_edit_customer"]').select2(
            {
                ajax: {
                    url: '/Customer/Filter',
                    ...
                    processResults: function (res) {
                        var data = $.map(res.items, function (item, i) {
                            ...
                            return {
                                id: item.id,
                                text: App.isNullOrEmpty(item.fullName) ?  item.lastName + ' ' + item.firstName : item.fullName
                            }
                        });
                        data.unshift({
                            id: 0,
        ...
        $('[name="kt_form_edit_saleStaff"]').select2(
            {
                ajax: {
                    url: '/Staff/Filter',
                    data: function (params) {
