...
var ProductType = function () {
    ...
    let validator = FormValidation.formValidation(modal_form[0], {
        fields: {
            kt_modal_edit_form_name: { validators: { notEmpty: { message: "vui lòng nhập tên nhóm hàng" } } }
        },
        plugins: {
    ...
    let initialComponents = () => {
        ...
        $('button[name="btnCreate"]').click(function (e) {
            ...
            editingDataRow = null;
            modal_header_text.text("Thêm một nhóm hàng");
        ...
        $('#kt_modal_edit_submit').click(function (e) {
            ...
            $('[name="kt_modal_edit_form_name"]').focus();
        });
        ...
        $('[name="kt_modal_edit_form_parentId"]').select2(
            {
                ajax: {
    ...
    let initialDatatable = function () {
        ...
        datatableOption.ajax.url = "/ProductType/DataTableGetList";
        ...
        datatableOption.columns = [
            ...
            { "data": "name", "name": "name", "autoWidth": true, "title": "Tên nhóm hàng" },
            { "data": "parentName", "name": "parentName", "autoWidth": true, "title": "Tên nhóm hàng cha" },
            {
                "data": "sortOrder", "name": "sortOrder", "autoWidth": true, "title": "Thứ tự", "render": function (data, type, full, meta) {
        ...
        $('#dtTable tbody').on('click', 'a.edit', function (e) {
            ...
            $('input[data-field="Name"]').val(!App.isNullOrEmpty(selectedDataRow.name) ? selectedDataRow.name.replaceAll('-','') : "");
            $('input[data-field="SortOrder"]').val(selectedDataRow.sortOrder);

            if (!App.isNullOrEmpty(selectedDataRow.parentId)) {
                $('[name="kt_modal_edit_form_parentId"]').append('<option value="' + selectedDataRow.parentId + '">' + selectedDataRow.parentName + '</option>').trigger('change');
            } else {
                $('[name="kt_modal_edit_form_parentId"]').append('<option value="0">Không chọn</option>').trigger('change');
            }

            modal_header_text.text("Cập nhật nhóm hàng");
    ...
    let resetForm = () => {
        modal_form[0].reset();
        setTimeout(function () { $('input[name="kt_modal_edit_form_name"]').focus() }, 500);
        $('[name="kt_modal_edit_form_parentId"]').val(0).trigger('change');
    }

    function deleteDataRows(dataRows) {
        App.deleteDataConfirm({ ids: dataRows.map((item) => item.id) }, "/ProductType/DeleteByIds", dtTable, null)
        .then(function () {
            dtTable.draw();
            App.showHideButtonDelete(false);
        });
