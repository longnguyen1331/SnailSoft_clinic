...
var Unit = function () {
    ...
    let validator = FormValidation.formValidation(modal_form[0], {
        fields: {
            kt_modal_edit_form_code: { validators: { notEmpty: { message: "vui lòng nhập mã đơn vị tính" } } },
            kt_modal_edit_form_name: { validators: { notEmpty: { message: "vui lòng nhập tên đơn vị tính" } } }
        },
        plugins: {
    ...
    let initialComponents = () => {
        ...
        $('button[name="btnCreate"]').click(function (e) {
            ...
            editingDataRow = null;
            modal_header_text.text("Thêm một đơn vị tính");
                ...
                validator.validate().then(function (e) {
                    "Valid" == e ?
                        (
                            ...
                            App.sendDataToURL("/Unit/Save", data, "POST")
                            .then(function (res)
                            {
    ...
    let initialDatatable = function () {
        ...
        datatableOption.ajax.url = "/Unit/DataTableGetList";
        ...
        datatableOption.columns = [
            ...
            { "data": "code", "name": "code", "autoWidth": true, "title": "Mã đơn vị tính" },
            { "data": "name", "name": "name", "autoWidth": true, "title": "Tên đơn vị tính" },
            {
        ...
        $('#dtTable tbody').on('click', 'a.edit', function (e) {
            ...
            modal_header_text.text("Cập nhật đơn vị tính");
            modal_form_submit_text.text("Cập nhật");
            setTimeout(function () { $('input[name="kt_modal_edit_form_code"]').focus() }, 500);

            modal.modal('show');
            $('[name="kt_modal_edit_form_status"]').val(parseInt(selectedDataRow.status)).trigger('change');
        });
    ...
    let resetForm = () => {
        modal_form[0].reset();
        setTimeout(function () { $('input[name="kt_modal_edit_form_code"]').focus() }, 500);
        $('[name="kt_modal_edit_form_status"]').val(0).trigger('change');
    }

    function deleteDataRows(dataRows) {
        App.deleteDataConfirm({ ids: dataRows.map((item) => item.id) }, "/Unit/DeleteByIds", dtTable, null)
        .then(function () {
            dtTable.draw();
            App.showHideButtonDelete(false);
        });
