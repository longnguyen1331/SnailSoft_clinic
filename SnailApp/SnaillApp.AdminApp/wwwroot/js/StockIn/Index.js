...
var StockIn = function () {
    ...
    let initialComponents = () => {
        ...
        $('[name="btnCreate"]').click(function (e) {
            e.preventDefault();
            window.location.href = "/Stockin/Edit";
        });
    ...
    let initialDatatable = function () {
        ...
        datatableOption.ajax.url = "/StockIn/DataTableGetList";
        ...
        $('#dtTable tbody').on('click', 'a.detail', function (e) {
            ...
            if (row.child.isShown()) {
                ...
                let html = '<div style="display:flex"><table cellpadding="5" cellspacing="0" border="0" style="width:40%;padding-left:50px;">\
                            <thead><tr><td><b>STT</b></td>\
                            <td><b>Mã Hàng hóa</b></td>\
                            <td><b>Tên Hàng hóa</b></td>\
                            <td><b>SL</b></td>\
                            <td style="padding:5px;"><b>Tiền</b></td>\
                            </tr></thead><tbody>';
                App.sendDataToURL("/StockIn/GetManageListDetailPaging", { Id: selectedDataRow.id, Data: null }, "POST", true)
                    .then(function (order) {
                        if (order.isSuccessed && order.resultObj != null) {
                            ...
                            $.each(order.resultObj, function (index, e) {
                                html += '<tr><td>' + (index + 1) + '</td>' +
                                    ...
                                    '<td>' + e.qty + '</td>' +
                                    '<td style="padding:5px;">' + Util.dinhDangTien(e.total) + '</td>' +
                                    '</tr>';
                            });
                            ...
                            html += '<table style="width:30%;">\
                                        <tr style="background:#ccc;"><td style="padding:5px;">Số món</td> <td style="padding:5px;">'+ order.resultObj.length + '</td> <tr>\
                                        <tr style="background:#ccc;"><td style="padding:5px;">Tổng tiền</td> <td style="padding:5px;">'+ Util.dinhDangTien(selectedDataRow.totalMoney) + '</td> <tr></table>';

                            html += '<table style="width:30%;">\
                                        <tr><td style="padding:5px;">Ghi chú <td>  <tr>\
                                        <tr><td style="padding:5px;"><textarea id="des'+ selectedDataRow.id+'" style="width:100%;">'+ selectedDataRow.description + '</textarea><button data-id="' + selectedDataRow.id+'" type="button" class="btn btn-light me-3 saveDescription">Lưu</button><td><tr></table></div>';
                            row.child(html).show();
                            saveDescriptionById();
                            tr.addClass('shown');
                        }
    ...
    function saveDescriptionById() {
        //Get template phiếu thu 
        $('.saveDescription').click(function () {
            let id = $(this).data('id');
            App.sendDataToURL("/StockIn/UpdateDescriptionById", { Id: id, Data: $('#des' + id).val() }, "POST")
                .then(function (res) {
    ...
    function print(data) {
        ...
        //Get template phiếu thu 
        App.sendDataToURL("/DocTemplate/GetContentByDocTemplateTypeCode", { Id: null, Data: { DocTemplateCode: "PN" } }, "POST")
            .then(function (res) {
                ...
                if (res.isSuccessed && res.resultObj != null) {
                    ...
                    App.sendDataToURL("/StockIn/GetManageListDetailPaging", { Id: data.id, Data: null }, "POST")
                        .then(function (stockIn) {
                            console.log(stockIn)
                            if (stockIn.isSuccessed && stockIn.resultObj != null) {
                                $("#page-print").empty();
                                ...
                                res.resultObj.content = res.resultObj.content.replace('{{ngay_nhap}}', data.dateString);
                                res.resultObj.content = res.resultObj.content.replace('{{so_phieu}}', data.code);
                                res.resultObj.content = res.resultObj.content.replace('{{nguoi_nhap}}', data.saleStaffName);
                                res.resultObj.content = res.resultObj.content.replace('{{nha_cung_cap}}', data.customerName);
                                ...
                                res.resultObj.content = res.resultObj.content.replace('{{trang_thai_thanh_toan}}', Util.dinhDangTien(data.totalMoney));



                                //table body
                                let body = '', qty = 0;
                                $.each(stockIn.resultObj, function (index, item) {
                                    qty += item.qty;
    ...
    function openDetail(id) {
        ...
        console.log(id);
        App.sendDataToURL("/StockIn/GetManageListDetailPaging", { Id: id, Data: null }, "POST")
            .then(function (res) {

                if (res.isSuccessed && res.resultObj != null && res.resultObj.length > 0) {
                    $.each(res.resultObj, function (index, item) {

                        let html = '<tr id="tag_' + item.id + '">\
                            <td>\
                                <div class="d-flex justify-content-start flex-column">\
                                    <a href="#!" class="text-dark fw-bolder text-hover-primary fs-6">'+ item.productSKUName + '</a>\
                                    <span class="text-muted fw-bold text-muted d-block fs-7">Mã : '+ item.productSKUCode + '</span>\
                                </div>\
                            </td>\
                            <td class="text-center">\
                                <span class="text-muted">' + item.qty + '</span>\
                            ...
                            <td class="text-center">\
                                <span class="text-muted">' + Util.dinhDangTien(item.buyingPrice) + '</span>\
                            </td>\
                        </tr>';
                        $('#kt_view_detail_fields tbody').append(html);
                        $('.tien-VND').autoNumeric('init', initialTienVNDOption());
                    });
    ...
    function deleteDataRows(dataRows) {
        App.deleteDataConfirm({ ids: dataRows.map((item) => item.id) }, "/StockIn/DeleteByIds", dtTable, null)
        .then(function () {
            dtTable.draw();
            App.showHideButtonDelete(false);
        });
