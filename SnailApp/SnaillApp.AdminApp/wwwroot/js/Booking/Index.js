...
var Booking = function () {
    ...
    let validator = FormValidation.formValidation(modal_form[0], {
        fields: {
            kt_modal_edit_form_name: { validators: { notEmpty: { message: "vui lòng nhập tên" } } },
            kt_modal_edit_form_email: { validators: { notEmpty: { message: "vui lòng nhập email" } } },
            kt_modal_edit_form_phoneNumber: { validators: { notEmpty: { message: "vui lòng nhập số điện thoại" } } },
            kt_modal_edit_form_dateContact: { validators: { notEmpty: { message: "vui lòng nhập ngày hẹn" } } }
        },
        plugins: {
    ...
    let initialComponents = () => {
        ...
        $('#kt_modal_edit_submit').click(function (e) {
                ...
                validator.validate().then(function (e) {
                    "Valid" == e ?
                        (
                                    ...
                                    switch (fieldName) {
                                        case "PetWeight":
                                            if (!isNaN(parseFloat($(el).val()))) {
                                                result[$(el).data("field")] = parseFloat($(el).val());
                                            }
                                            else {
                                                result[$(el).data("field")] = 0;
                                            }
                            ...
                            App.sendDataToURL("/Booking/Save", data, "POST")
            ...
            $('input[name="kt_modal_edit_form_name"]').focus();
        });
        ...
        App.initSelect2Base($('[name="kt_modal_edit_form_petServiceSelectionId"]'), '/PetServiceSelection/Filter');
    };

    let initialDatatable = function () {
        var datatableOption = initialDatatableOption();
        datatableOption.ajax.url = "/Booking/DataTableGetList";
        ...
        datatableOption.columns = [
            ...
            { "data": "petName", "name": "petName", "autoWidth": true, "title": "Pet name" },
            { "data": "petWeight", "name": "petWeight", "autoWidth": true, "title": "Pet weight" },
            { "data": "strDateContact", "name": "strDateContact", "autoWidth": true, "title": "Ngày hẹn" },
            {   
                "data": "petServiceSelection", "name": "petServiceSelection", "autoWidth": true, "title": "Service request",
                "render": function (data, type, full, meta) {
                    return '<span class="badge badge-light-success fs-8 fw-bolder">' + (data ? data.name : '') + '</span>';
                }
            },
            { "data": "specialNote", "name": "specialNote", "autoWidth": true, "title": "Ghi chú" },
            {
        ...
        $('#dtTable tbody').on('click', 'a.edit', function (e) {
            ...
            $('input[data-field="Email"]').val(selectedDataRow.email);
            $('input[data-field="PhoneNumber"]').val(selectedDataRow.phoneNumber);
            $('input[data-field="PettName"]').val(selectedDataRow.petName);
            $('input[data-field="PetWeight"]').val(selectedDataRow.petWeight);
            $('input[data-field="DateContact"]').val(selectedDataRow.strDateContact);
            $('input[data-field="SpecialNote"]').val(selectedDataRow.specialNote);

            if (selectedDataRow.petServiceSelection) {
                $('[name="kt_modal_edit_form_petServiceSelectionId"]').append("<option value=" + selectedDataRow.petServiceSelection.id + " selected>" + selectedDataRow.petServiceSelection.name + "</option>").trigger('change');
            }
            else {
                $('[name="kt_modal_edit_form_petServiceSelectionId"]').val(null).trigger('change');
            }

            modal_header_text.text("Cập nhật ");
    ...
    function deleteDataRows(dataRows) {
        App.deleteDataConfirm({ ids: dataRows.map((item) => item.id) }, "/Booking/DeleteByIds", dtTable, null)
        .then(function () {
            dtTable.draw();
            App.showHideButtonDelete(false);
        });
