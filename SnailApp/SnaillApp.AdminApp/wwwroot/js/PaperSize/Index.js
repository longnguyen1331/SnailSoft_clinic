...
var PaperSize = function () {
    ...
    let validator = FormValidation.formValidation(modal_form[0], {
        fields: {
            ...
            kt_modal_edit_form_value: { validators: { notEmpty: { message: "vui lòng chọn chiều rộng" } } }
        },
        plugins: {
    ...
    let initialComponents = () => {
        ...
        $('[name="btnCreate"]').click(function (e) {
            ...
            editingDataRow = null;
            modal_header_text.text("Thêm một khổ giấy");
                ...
                validator.validate().then(function (e) {
                    "Valid" == e ?
                        (
                            ...
                            App.sendDataToURL("/PaperSize/Save", data, "POST")
    ...
    let initialDatatable = function () {
        ...
        datatableOption.ajax.url = "/PaperSize/DataTableGetList";
        ...
        datatableOption.columns = [
            ...
            { "data": "value", "name": "value", "autoWidth": true, "title": "Chiều rộng" },
            {
        ...
        $('#dtTable tbody').on('click', 'a.edit', function (e) {
            ...
            $('input[data-field="Value"]').val(selectedDataRow.value);
    ...
    function deleteDataRows(dataRows) {
        App.deleteDataConfirm({ ids: dataRows.map((item) => item.id) }, "/PaperSize/DeleteByIds", dtTable, null)
        .then(function () {
            dtTable.draw();
            App.showHideButtonDelete(false);
        });
