...
var Customer = function () {
    ...
    let validator = FormValidation.formValidation(edit_form[0], {
        fields: {
            ...
            edit_form_genderId: { validators: { notEmpty: { message: "vui lòng chọn giới tính" } } }            
        },
        plugins: {
    ...
    let initialComponents = () => {
                ...
                validator.validate().then(function (e) {
                    ...
                    "Valid" == e ?
                        (
                            ...
                            App.sendDataFileToURL("/Customer/SaveProfileDetail", formData, "POST")
                                .then(function (res) {
        ...
        $('#btnDeleteAvatar').click(function (e) {
            ...
            if (editingData != null && editingData.id != null) {
                App.sendDataToURL("/Customer/DeleteAvatarByUserId?userId=" + editingData.id, null, "GET")
                    .then(function (res) {
                        if (!res.isSuccessed) {
                            Swal.fire(App.swalFireErrorDefaultOption(res.message));
                        }
        ...
        App.initSelect2Base($('[name="edit_form_genderId"]'), '/Gender/Filter');
        if (editingData && editingData.gender) {
            $('[name="edit_form_genderId"]').append("<option value=" + editingData.gender.id + " selected>" + editingData.gender.name + "</option>").trigger('change');
        }
