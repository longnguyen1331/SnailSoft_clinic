...
var CommissionDiscount = function () {
    let dtTable = null;
                ...
                validator.validate().then(function (e) {
                    "Valid" == e ?
                        (
                                ...
                                if (fieldName) {
                                    ...
                                    switch (fieldName) {
                                        ...
                                        case "DiscountPercent":
                                            result[$(el).data("field")] = ($(el).val() ? parseFloat($(el).val()) : 0);
                                            break;
                            ...
                            App.sendDataToURL("/CommissionDiscount/Save", data, "POST")
                            .then(function (res)
                            {
    ...
    let initialDatatable = function () {
        ...
        datatableOption.ajax.url = "/CommissionDiscount/DataTableGetList";
        ...
        $('#dtTable tbody').on('click', 'a.edit', function (e) {
            ...
            $('input[data-field="Commission"]').val(editingDataRow.commission);
            $('input[data-field="CommissionPercent"]').val(editingDataRow.CommissionPercent);
            $('input[data-field="Discount"]').val(editingDataRow.discount);
            $('input[data-field="DiscountPercent"]').val(editingDataRow.discountPercent);
            $('input[data-field="FromDate"]').val(editingDataRow.strFromDate);
            $('input[data-field="ToDate"]').val(editingDataRow.strToDate);
            $('input[data-field="IsApply"]').prop('checked', editingDataRow.isApply);
    ...
    function deleteDataRows(dataRows) {
        App.deleteDataConfirm({ ids: dataRows.map((item) => item.id) }, "/CommissionDiscount/DeleteByIds", dtTable, null)
        .then(function () {
            dtTable.draw();
            App.showHideButtonDelete(false);
        });
