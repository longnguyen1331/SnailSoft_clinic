...
var Store = function () {
    ...
    let validator = FormValidation.formValidation(modal_form[0], {
        fields: {
            kt_modal_edit_form_code: { validators: { notEmpty: { message: "vui lòng nhập mã cửa hàng" } } },
            kt_modal_edit_form_name: { validators: { notEmpty: { message: "vui lòng nhập tên cửa hàng" } } },
    ...
    let initialComponents = () => {
        ...
        $('[name="btnCreate"]').click(function (e) {
            ...
            editingDataRow = null;
            modal_header_text.text("Thêm một cửa hàng");
        ...
        $('[name="kt_modal_edit_form_status"]').on("change", function () {
            validator.revalidateField("kt_modal_edit_form_status");
        });
                ...
                validator.validate().then(function (e) {
                    "Valid" == e ?
                        (
                            ...
                            App.sendDataToURL("/Store/Save", data, "POST")
        ...
        App.initSelect2Base($('[name="kt_modal_edit_form_status"]'), '/AppUserStatus/Filter');
    };

    let initialDatatable = function () {
        var datatableOption = initialDatatableOption();
        datatableOption.ajax.url = "/Store/DataTableGetList";
        ...
        datatableOption.columns = [
            ...
            { "data": "code", "name": "code", "autoWidth": true, "title": "Mã cửa hàng" },
            { "data": "name", "name": "name", "autoWidth": true, "title": "Tên cửa hàng" },
            { "data": "storeStatusId", "name": "storeStatusId", "autoWidth": true, "title": "storeStatusId" },
            {   
                "data": "storeStatusName", "name": "storeStatusName", "autoWidth": true, "title": "Trạng thái",
                "render": function (data, type, full, meta) {
                    return '<span class="badge badge-light-success fs-8 fw-bolder">' + data + '</span>';
                }
            },
            {
                "data": "phone", "name": "phone", "autoWidth": true, "title": "Điện thoại", "render": function (data, type, full, meta) {
                    return '<span class="text-muted">' + data + '</span>';
                }
            },
            {
                "data": "email", "name": "email", "autoWidth": true, "title": "Email", "render": function (data, type, full, meta) {
                    return '<span class="text-muted">' + data + '</span>';
                }
            },
            {
                "data": "address", "name": "address", "autoWidth": true, "title": "Địa chỉ", "render": function (data, type, full, meta) {
                    return '<span class="text-muted">' + data + '</span>';
                }
            },
            {
                "data": "dayOfActive", "name": "dayOfActive", "autoWidth": true, "title": "Ngày hoạt động", "render": function (data, type, full, meta) {
                    return '<span class="text-muted">' + data + '</span>';
                }
            },
            {
        ...
        $('#dtTable tbody').on('click', 'a.edit', function (e) {
            ...
            $('input[data-field="Phone"]').val(selectedDataRow.phone);
            $('input[data-field="Email"]').val(selectedDataRow.email);
            $('input[data-field="Address"]').val(selectedDataRow.address);
            $('input[data-field="Code"]').val(selectedDataRow.code);
            $('input[data-field="Name"]').val(selectedDataRow.name);
            
            $('[name="kt_modal_edit_form_status"]').append("<option value=" + selectedDataRow.storeStatusId + " selected>" + selectedDataRow.storeStatusName + "</option>").trigger('change');

            modal_header_text.text("Cập nhật cửa hàng");
    ...
    let resetForm = () => {
        modal_form[0].reset();
        setTimeout(function () { $('input[name="kt_modal_edit_form_code"]').focus() }, 500);
        $('[name="kt_modal_edit_form_status"]').val(null).trigger('change');
    }

    function deleteDataRows(dataRows) {
        App.deleteDataConfirm({ ids: dataRows.map((item) => item.id) }, "/Store/DeleteByIds", dtTable, null)
        .then(function () {
            dtTable.draw();
            App.showHideButtonDelete(false);
        });
