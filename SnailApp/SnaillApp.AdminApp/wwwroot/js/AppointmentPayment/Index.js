...
var AppointmentPayment = function () {
    ...
    let initialDatatable = function () {
        ...
        datatableOption.ajax.url = "/AppointmentPayment/DataTableGetList";
        ...
        datatableOption.columns = [
            ...
            },
            {
                "data": "status", "name": "status", "autoWidth": true, "title": "Status", "render": function (data, type, full, meta) {
                    ...
                    switch (data) {
                        case 0:
                            html = '<div class="d-flex align-items-center border-left border-4  text-cancel"><i class="bx bx-radio-circle-marked bx-burst bx-rotate-90 align-middle font-18 me-1"></i><span class="mb-0l"> Unpaid </span></div>';
                            break;
                        case 1:
                            html = '<div class="d-flex align-items-center border-left border-4 text-checkout"><i class="bx bx-radio-circle-marked bx-burst bx-rotate-90 align-middle font-18 me-1"></i><span class="mb-0"> Paid</span></div>';
                            break;
                        case 2:
                            html = '<div class="d-flex align-items-center border-left border-4  text-booking"><i class="bx bx-radio-circle-marked bx-burst bx-rotate-90 align-middle font-18 me-1"></i><span class="mb-0">Debit</span></div>';
                            break;
                        default:
                            break;
                    }
            ...
            },
            {
                width: "120px", "title": "Active", "render": function (data, type, full, meta) {
                    ...
                    if (user.roles.isAllowEdit == true) {
                        html += '<a href="/AppointmentPayment/Update?id=' + full.id +'"  class="edit btn btn-outline-orange ms-2">\
    ...
    function deleteDataRows(dataRows) {
        App.deleteDataConfirm({ ids: dataRows.map((item) => item.id) }, "/AppointmentPayment/DeleteByIds", dtTable, null)
        .then(function () {
            dtTable.draw();
            App.showHideButtonDelete(false);
        });
