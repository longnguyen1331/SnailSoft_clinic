...
namespace SnailApp.AdminApp.Controllers
{
    public class PaperSizeController : Controller
    {
        ...
        public PaperSizeController(IPaperSizeApiClient paperSizeApiClient, IUserApiClient userApiClient)
        {
        ...
        {
            var model = new PaperSizeViewModel();
        ...
        public async Task<IActionResult> DataTableGetList(int? draw, int? start, int? length, string textSearch)
        {
            ...
            var request = new ManagePaperSizePagingRequest()
            {
            ...
            var paperSizeApiClient = await _paperSizeApiClient.GetManageListPaging(request);

            return Json(new
            {
                draw = draw,
                recordsFiltered = paperSizeApiClient.TotalRecords,
                recordsTotal = paperSizeApiClient.TotalRecords,
                data = paperSizeApiClient.Items
            });
        }
        ...
        public async Task<IActionResult> DeleteByIds([FromBody] DeleteRequest request)
        {
            ...
            var result = await _paperSizeApiClient.DeleteByIds(request);
        ...
        [HttpPost]
        public async Task<IActionResult> Save([FromBody] AddEditRequest<PaperSizeRequest> rq)
        {
            ...
            if (rq != null)
            {
                ...
                result = await _paperSizeApiClient.AddOrUpdateAsync(rq.Data);
            }
            else
            {
        ...
        public async Task<FileResult> ExportToExcel()
        {
            ...
            var data = await _paperSizeApiClient.GetManageListPaging(request);
            ...
            if (data != null)
            {
                ...
                if (data.Items.Count() > 0)
                {
                    foreach (var item in data.Items)
                    {
                        ...
                        {
                            propertyInfo = typeof(PaperSizeDto).GetProperty(exportedField.Code);
