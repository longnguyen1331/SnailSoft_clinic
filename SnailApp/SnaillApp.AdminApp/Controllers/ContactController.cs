...
namespace SnailApp.AdminApp.Controllers
{
    public class ContactController : Controller
    {
        ...
        public IActionResult Index()
        {
            ...
            model.PageTitle = "Liên hệ";
            model.Breadcrumbs = new List<string>() { "Website", "Liên hệ" };
        ...
        public async Task<IActionResult> DataTableGetList(int? draw, int? start, int? length, string textSearch)
        {
            ...
            var request = new ManageContactPagingRequest()
            {
            ...
            var contactApiClient = await _contactApiClient.GetManageListPaging(request);

            return Json(new
            {
                draw = draw,
                recordsFiltered = contactApiClient.TotalRecords,
                recordsTotal = contactApiClient.TotalRecords,
                data = contactApiClient.Items
            });
        }
        ...
        public async Task<IActionResult> DeleteByIds([FromBody] DeleteRequest request)
        {
            ...
            var result = await _contactApiClient.DeleteByIds(request);
        ...
        public async Task<IActionResult> Save([FromBody] AddEditRequest<ContactRequest> rq)
        {
            ...
            if (rq != null)
            {
                ...
                result = await _contactApiClient.AddOrUpdateAsync(rq.Data);
            }
            else
            {
        ...
        public async Task<FileResult> ExportToExcel()
        {
            ...
            var data = await _contactApiClient.GetManageListPaging(request);
            ...
            if (data != null)
            {
                ...
                if (data.Items.Count() > 0)
                {
                    foreach (var item in data.Items)
                    {
                        ...
                        {
                            propertyInfo = typeof(ContactDto).GetProperty(exportedField.Code);
